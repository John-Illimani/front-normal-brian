...
import { 
  BarChart3, Users, FileSpreadsheet, Search,
  ArrowUpDown, Eye, X, FileText, Camera,
  FileCheck, FileDown, Loader2, ShieldCheck,
  Filter
} from 'lucide-react';
import * as XLSX from 'xlsx';
import jsPDF from 'jspdf';
import { getCurriculumByUserIdRequest, getAtletasRequest } from '../../services/api.jsx';
...
export const ReportesUsuarios = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const rawAtletas = atletasRes.status === 'fulfilled' ? (atletasRes.value.data?.data || atletasRes.value.data || []) : [];
  ...
  const fetchFichaEstudiante = async (user) => {
    ...
    return {
      perfil: {
        id: user.id,
        nombre_completo: `${user.nombre || ''} ${user.apellido || ''}`.trim() || user.username || 'Atleta Sin Registro',
  ...
  const processedUsers = useMemo(() => {
    return users
      .filter((u) => {
        ...
        if (filterCategoria !== 'todas' && cat !== filterCategoria) return false;

        if (filterDatos === 'con_datos' && !tieneFicha) return false;
        if (filterDatos === 'sin_datos' && tieneFicha) return false;

        const fullName = `${datosAtleta?.nombre_completo || u.nombre || ''} ${u.apellido || ''}`.toLowerCase();
        const ci = (u.ci || '').toLowerCase();
        const username = (u.username || '').toLowerCase();
        const search = searchTerm.toLowerCase();

        return fullName.includes(search) || ci.includes(search) || username.includes(search);
      })
      .map(u => ({
        ...u,
      ...
      .sort((a, b) => {
        const nameA = (a.datosAtleta?.nombre_completo || `${a.nombre || ''} ${a.apellido || ''}`).trim() || a.username || '';
        const nameB = (b.datosAtleta?.nombre_completo || `${b.nombre || ''} ${b.apellido || ''}`).trim() || b.username || '';

        return sortOrder === 'asc' ? nameA.localeCompare(nameB) : nameB.localeCompare(nameA);
      });
  }, [users, mapaAtletas, filterCategoria, filterDatos, searchTerm, sortOrder]);
  ...
  const getFilteredExportList = (filters) => {
    const filtrados = users.filter((u) => {
      ...
      const cat = datosAtleta?.categoria || u.categoria || 'Junior';
      if (filters.categoria !== 'todas' && cat !== filters.categoria) return false;
    ...
    return filtrados.map((u, idx) => {
      return {
        correlativo: idx + 1,
        nombre_completo: u.datosAtleta?.nombre_completo || `${u.nombre || ''} ${u.apellido || ''}`.trim() || u.username,
        ci: u.ci || 'Sin CI',
  ...
  const generarReportePDFDirecto = (listado, tituloReporte) => {
    ...
    doc.setFontSize(9);
    doc.text(`NÓMINA OFICIAL DE ATLETAS — ${tituloReporte} | EMISIÓN: ${new Date().toLocaleDateString()}`, 14, 35);
  ...
  const handleExportPDFFromModal = () => {
    ...
    generarReportePDFDirecto(listado, `Reporte_Atletas_${exportFilters.categoria}`);
  ...
  const handleExportExcelFromModal = () => {
    ...
    XLSX.writeFile(wb, `Reporte_Atletas_${exportFilters.categoria.toUpperCase()}.xlsx`);
    setExportModalOpen(false);
  };
    ...
    <div className="space-y-6 font-sans print:p-0">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 p-6 rounded-3xl text-white shadow-xl relative overflow-hidden print:hidden">
        <div className="relative z-10">
          ...
          <h1 className="text-2xl font-extrabold tracking-tight">Reportes de Atletas y Fichas Técnicas</h1>
          <p className="text-xs text-slate-300 mt-1 max-w-xl">
            Acceso centralizado para supervisar registros, previsualizar documentos adjuntos y generar reportes oficiales.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-1 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-slate-400">Total Registrados</p>
            <p className="text-2xl font-black text-slate-900 mt-1">{totalUsuarios}</p>
            <span className="text-[10px] text-slate-400 font-medium">Usuarios en Sistema</span>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-slate-500 text-sm">Cargando nómina de atletas...</div>
        ) : (
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 text-slate-500 font-bold uppercase tracking-wider border-b border-slate-100">
                <tr>
                  ...
                  <th className="py-3.5 px-6 text-center print:hidden">Acciones</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {processedUsers.length === 0 ? (
                  ...
                  processedUsers.map((u, index) => {
                    const nombreCompleto = u.datosAtleta?.nombre_completo || `${u.nombre || ''} ${u.apellido || ''}`.trim() || u.username;
                    ...
                    return (
                      <tr key={u.id || index} className="hover:bg-slate-50">
                        <td className="py-3.5 px-6 font-bold text-slate-400">{index + 1}</td>
                        <td className="py-3.5 px-6">
                          <p className="font-bold text-slate-900">{nombreCompleto}</p>
                          <p className="text-[11px] text-indigo-600">@{u.username}</p>
                        </td>
                        ...
                        <td className="py-3.5 px-6 font-semibold text-slate-800">
                          <span className="bg-slate-100 px-2 py-1 rounded-md border border-slate-200">
                            {u.datosAtleta?.categoria || u.categoria || 'Junior'}
                          </span>
                        </td>
                        <td className="py-3.5 px-6">
                          {tieneFicha ? (
                        ...
                        <td className="py-3.5 px-6 text-center print:hidden flex items-center justify-center gap-2">
                          <button
                            ...
                            title={tieneFicha ? "Ver Ficha" : "Sin datos de ficha técnica registrados"}
                          >
                            {isRowLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Eye className="w-3.5 h-3.5" />}
                            <span>Ver Ficha</span>
                          </button>
                          ...
                          <button
                            ...
                            title={tieneFicha ? "Generar PDF Individual" : "Sin datos de ficha técnica registrados"}
                          >
                            {isRowLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <FileDown className="w-3.5 h-3.5" />}
                            <span>PDF</span>
                          </button>
                        </td>
                      </tr>
          ...
          <div className="bg-white border-4 border-[#c5a86f] w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden flex flex-col">
            ...
            <div className="flex items-center justify-between px-6 py-4 bg-slate-900 text-white border-b-2 border-[#c5a86f]">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-sm font-bold tracking-wide">Generar Reporte y Filtros</h3>
              </div>
              <button
            ...
            <div className="p-6 space-y-4 text-xs text-slate-700">
              ...
              <div>
                <label className="font-bold text-slate-600 block mb-1">Filtrar por Búsqueda (Nombre o CI):</label>
                <input
                  type="text"
              ...
              <div>
                <label className="font-bold text-slate-600 block mb-1">Seleccionar Categoría:</label>
                <select
              ...
              <div>
                <label className="font-bold text-slate-600 block mb-1">Estado de Ficha:</label>
                <select
                  value={exportFilters.fichaStatus}
                  onChange={(e) => setExportFilters({ ...exportFilters, fichaStatus: e.target.value })}
                  className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                >
                  <option value="todos">Todos los Registros</option>
              ...
              <div className="bg-slate-50 p-3 rounded-xl border text-[11px] text-slate-500">
                Total de registros listos para exportar: <strong className="text-emerald-800">{getFilteredExportList(exportFilters).length}</strong>
              </div>
          ...
          <div className="bg-white border border-slate-200 w-full max-w-4xl max-h-[90vh] rounded-3xl shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 bg-slate-900 text-white">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center font-bold text-lg">
                  {(atletaCurriculum.perfil?.nombre_completo || selectedAtleta.username || 'A').charAt(0)}
                </div>
                <div>
                  <h3 className="text-base font-bold">{atletaCurriculum.perfil?.nombre_completo || selectedAtleta.username}</h3>
                  <p className="text-xs text-indigo-300">CI: {selectedAtleta.ci || 'Sin CI'} | @{selectedAtleta.username}</p>
                </div>
              </div>
            ...
            <div className="p-6 overflow-y-auto space-y-6 text-xs text-slate-700">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="bg-slate-50 p-2.5 border rounded-2xl text-center space-y-1.5">
                    <span className="font-bold text-[11px] text-slate-600 block">EN COMPETENCIA</span>
                    <div className="w-full h-36 bg-slate-200 rounded-xl overflow-hidden flex items-center justify-center">
                      {atletaCurriculum.perfil?.foto_en_competencia_url ? (
                        <img src={atletaCurriculum.perfil.foto_en_competencia_url} alt="Competencia" className="w-full h-full object-cover" />
                      ) : (
                  ...
                  <div className="bg-slate-50 p-2.5 border rounded-2xl text-center space-y-1.5">
                    <span className="font-bold text-[11px] text-slate-600 block">EN PODIO / PREMIACIÓN</span>
                    <div className="w-full h-36 bg-slate-200 rounded-xl overflow-hidden flex items-center justify-center">
                      {atletaCurriculum.perfil?.foto_en_podio_premiacion_url ? (
                        <img src={atletaCurriculum.perfil.foto_en_podio_premiacion_url} alt="Podio" className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-slate-400">Sin foto</span>
                      )}
                    </div>
                  ...
                  <div className="bg-slate-50 p-2.5 border rounded-2xl text-center space-y-1.5">
                    <span className="font-bold text-[11px] text-slate-600 block">REPRESENTANDO A BOLIVIA</span>
                    <div className="w-full h-36 bg-slate-200 rounded-xl overflow-hidden flex items-center justify-center">
                      {atletaCurriculum.perfil?.foto_representando_a_bolivia_url ? (
                        <img src={atletaCurriculum.perfil.foto_representando_a_bolivia_url} alt="Bolivia" className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-slate-400">Sin foto</span>
                      )}
                    </div>
              ...
              <div className="bg-slate-50 p-4 border rounded-2xl grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div>
                  <span className="text-slate-400 font-bold block text-[10px]">CATEGORÍA</span>
                  <span className="font-black text-indigo-950 text-sm">{atletaCurriculum.perfil?.categoria || 'Junior'}</span>
                </div>
                <div>
                  <span className="text-slate-400 font-bold block text-[10px]">CLUB</span>
                  <span className="font-black text-slate-800">{atletaCurriculum.perfil?.club || 'Sin Club'}</span>
                </div>
                <div>
                  <span className="text-slate-400 font-bold block text-[10px]">ASOCIACIÓN</span>
                  <span className="font-black text-slate-800">{atletaCurriculum.perfil?.asociacion_departamental || 'N/A'}</span>
                </div>
                <div>
                  <span className="text-slate-400 font-bold block text-[10px]">ENTRENADOR</span>
                  <span className="font-black text-slate-800">{atletaCurriculum.perfil?.entrenador_actual || 'N/A'}</span>
                </div>
              </div>
                  ...
                  {[
                    ...
                    <div key={doc.num} className="bg-white p-3 border rounded-xl flex flex-col justify-between text-center space-y-2">
                      ...
                      <div className="p-2 bg-slate-50 border border-dashed rounded-lg min-h-[50px] flex items-center justify-center">
                        {doc.base64 ? (
                          <span className="font-bold text-indigo-900 truncate max-w-[150px]">{doc.name || 'Documento PDF'}</span>
                        ) : (
                          <span className="text-slate-400 text-[10px]">Sin adjunto</span>
                        )}
                      </div>
                      {doc.base64 && (
                        <button
                          onClick={() => handlePreviewPdf(doc.base64, doc.label)}
